...
Item {
    property var cfg_gpuTempSources: []
    property var cfg_gpuTempLabels: []
        ...
        Repeater {
            ...
            CheckBox {
                ...
                onCheckedChanged: {
                    ...
                    if (checked) {
                        idx = cfg_gpuTempSources.indexOf(model.keyid);
                        if (idx < 0) {
                            txtfld.itemAt(index).enabled = true;
                            cfg_gpuTempSources.push(model.keyid);
                            cfg_gpuTempLabels.push(displaylbl);
                        }
                    } else {
                        txtfld.itemAt(index).enabled = false;
                        idx = cfg_gpuTempSources.indexOf(model.keyid);
                        cfg_gpuTempSources.splice(idx, 1);
                        cfg_gpuTempLabels.splice(idx, 1);
                    }
                    cfg_gpuTempSourcesChanged();
                }
            }
        }
        Repeater {
            ...
            TextField {
                ...
                onEditingFinished: {
                    ...
                    idx = cfg_gpuTempSources.indexOf(model.keyid);
                    cfg_gpuTempLabels.splice(idx, 1, text);
                    cfg_gpuTempSourcesChanged();
                }
            }
        }
    ...
    Component.onCompleted: {
        ...
        sourcesWrk = plasmoid.configuration.gpuTempSources;
        labelsWrk = plasmoid.configuration.gpuTempLabels;
        cfg_gpuTempSources.length = 0;
        cfg_gpuTempLabels.length = 0;
            ...
            if (sensors[idx].endsWith("/name")) {
                ...
                if (cfgIdx < 0 ) {
                    ...
                    if (typeof(gpuLabel) == "undefined")
                        gpuLabel = "";
                    cfg_gpuTempSources.push(key);
                    cfg_gpuTempLabels.push(gpuLabel);
                    sources.append({keyid:key, name: gpuName, checked: true, label: gpuLabel});
                }
            }
        }
        plasmoid.configuration.gpuTempSources = cfg_gpuTempSources;
        plasmoid.configuration.gpuTempLabels = cfg_gpuTempLabels;
    }
}
